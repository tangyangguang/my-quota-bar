...
- 面板平铺为默认形态；账号和套餐卡（Agent Plan / Coding Plan）支持**折叠**（用户手动、状态持久化、新账号默认展开）。折叠不是隐藏：账号折叠时**一个套餐/服务占一行**（服务小图标 + 该服务各窗口/ASR·TTS 的带额度色剩余%，`ServiceSummaryRow`）；单个套餐卡折叠时收成一行摘要。数字与展开态同源同字号。折叠 chevron 展开态 hover 才显示、折叠态常显；**所有 chevron 必须严格对齐在面板最左 gutter 同一列（glyph 中心距弹窗左缘约 5pt），与账号人头图标保持约 9pt 间距，不得重叠**（账号 header 用 offset x=-16，服务标题行用 x=-24，两处参照系不同，改动 padding 时必须重新实测坐标）。
