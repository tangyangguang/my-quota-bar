...
- 面板平铺为默认形态；**所有服务卡（Agent Plan / Coding Plan / 语音）统一支持折叠**（用户手动、状态持久化、新服务默认展开），逻辑一致：点标题行折叠/展开、chevron hover 淡入/折叠常显、折叠态收成同一形态的 `QuotaSummaryLine` 摘要（语音为单个 ASR/TTS 指标）、折叠态不显示钉选热区（点标题行展开，点数据行才钉选）。折叠不是隐藏：账号折叠时**一个套餐/服务占一行**（服务小图标 + 该服务各窗口/ASR·TTS 的带额度色剩余%，`ServiceSummaryRow`）。数字与展开态同源同字号。折叠 chevron 展开态 hover 才显示、折叠态常显；**所有 chevron 必须严格对齐在面板最左 gutter 同一列（glyph 中心距弹窗左缘约 5pt），与账号人头图标保持约 9pt 间距，不得重叠**（账号 header 用 offset x=-16，服务标题行用 x=-24，两处参照系不同，改动 padding 时必须重新实测坐标）。
